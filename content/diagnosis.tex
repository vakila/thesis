...
%knowledge of the text of the given utterance (known beforehand in the IFCASL case), 
	...
	%and one of the intended outcomes of the IFCASL project is the extraction of a non-native variant lexicon from the L2 speech in the corpus \TODO{ref?}.
	...
	Once the requisite German-language resources are available in JSnoori, de-stress can easily be extended to perform on-the-fly segmentation of learner utterances. % (see \cref{sec:conclusion:future}).
...
%\section{Comparison of native and non-native speech}
...
\label{sec:diag:system}
	...
	%\TODO{The choices available in the tool are illustrated in \cref{fig:diag:system}. }
...
%	\begin{itemize}
%		\item{
%		\textit{Which diagnosis approach?}
%			\begin{itemize}
%				\item{COMPARISON
%					\begin{itemize}
%						\item{\textit{Which reference selection method?}
%							\begin{itemize}
%								\item{MANUAL
%									\begin{itemize}
%										\item{\textit{Whose choice?}
%											\begin{itemize}
%											\item TEACHER
%											\item STUDENT
%											\end{itemize}
...
%							\end{itemize}
%						}
%						\item{\textit{How many references?}
%							\begin{itemize}
%								\item{SINGLE}
%								\item{MULTIPLE}
%							\end{itemize}
...
%
%			
%			\node (approach) [decision] {Which diagnosis approach?};
%			%\node (comp) [process, below of=q1, xshift=-3cm] {COMPARISON};
%			%\node (class) [process, below of=q1, xshift=3cm] {CLASSIFICATION};
%			%\draw [arrow] (q1) -- (comp);
%			%\draw [arrow] (q1) -- (class);
%			
%			\node (ref) [decision, below of=approach, xshift=-3cm] {Which reference selection method?};
%			\node (features) [decision, below of=approach, xshift=3cm] {Which features?};
%			\draw [arrow] (approach) -- node[anchor=east, scale=1.3, xshift=-1mm] {COMPARISON} (ref);			
%			\draw [arrow] (approach) -- node[anchor=west, scale=1.3, xshift=1mm] {CLASSIFICATION} (features);
%			
%			
%%			\draw [arrow] (comp) -- (q2);
%%			\node (man) [process, below of=q2, xshift=-2cm] {MANUAL};
%%			\node (auto) [process, below of=q2, xshift=2cm] {AUTOMATIC};
%%			\draw [arrow] (q2) -- (man);
%%			\draw [arrow] (q2) -- (auto);
%%			
%
%			\node (choice) [decision, below of=ref, xshift=-3cm] {Whose choice?};
%			\draw [arrow] (ref) -- node[anchor=east, scale=1.3, xshift=-1mm] {MANUAL} (choice);
%
%			\node (nrefs) [decision, below of=choice, xshift=3cm] {How many references?};	
%
%			
%			\draw [arrow] (choice) to [bend left] node[anchor=east, scale=1.3] {STUDENT} (nrefs);
%			\draw [arrow] (choice) to [bend right] node [anchor=east, scale=1.3, xshift=-1mm] {TEACHER} (nrefs);
%			
%			\draw [arrow] (ref) to [bend left=60] node[anchor=west, scale=1.3, xshift=1mm] {AUTOMATIC} (nrefs);
%%			\draw [arrow] (man) -- (q3);
%%			\node (teach) [process, below of=q3, xshift=-2cm] {TEACHER};
%%			\node (stud) [process, below of=q3, xshift=2cm] {STUDENT};
%%			\draw [arrow] (q3) -- (teach);
%%			\draw [arrow] (q3) -- (stud);
%
%		\end{tikzpicture}		
...
%		
%		\caption[Overview of diagnosis options]{Overview of diagnosis options in de-stress \TODO{Redo as Tikz flowchart}}
