...
\begin{itemize}
\item{\Highlight{Classification} using machine learning [3]. Possible feature sets:
	\begin{itemize}
...
%		\end{itemize}
%	}
	\item{Syllable-level prosodic features  (extracted with JSnoori [2]):
		\begin{itemize}
		\item{Duration}
		\item{Fundamental frequency (F0)}
		\item{Intensity}
...
\item{\Highlight{Comparison} to reference (native-speaker) utterance(s). Options:
	\begin{itemize}
	\item{One-to-one learner-to-reference comparison using JSnoori [2]}
...
\vspace{1em}
  ...
  \item{\Highlight{Implicit feedback}:
  	\begin{itemize}
  	\item{\Highlight{Visual}:
	  	\begin{itemize}
	  	\item{Graphical visualizations of prosody (see \cref{fig:feedback})}
	  	\item{Text stylization (see \cref{fig:feedback})}
	  	\end{itemize}
	}
	\item{\Highlight{Auditory}: 
		\begin{itemize}
		\item{Original learner and reference utterances}
		\item{Prosodically modified learner utterance (using JSnoori [2])}
		\end{itemize}
  	}
  	\end{itemize}
  }
  \item{\Highlight{Self-assessment} - learner must complete questionnaire before receiving other feedback}
  \end{itemize}
...
\end{textblock}
  ...
  \begin{textblock}{7}(9, 16.75) {
\banner
} \end{textblock}
\begin{textblock}{6.75}(9.25,17){
  \LHead{\titlecolor Administrative interface for teachers/researchers}}
 \end{textblock}
 \begin{textblock}{7}(9, 17.75)
  \Textsize
  ...
  A simple graphical administrative interface allows a language teacher or CAPT researcher to create new exercises for students to complete, where each exercise features a specific combination of the various diagnostic methods and feedback types available in the system. 
