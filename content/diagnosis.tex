...
For the two most frequently observed classes in the data, [correct] and [incorrect], the following evaluation metrics were also computed:
			...
			\item{F-measure (F), also known as F-score or F$_1$-score: a metric which combines Precision and Recall by taking their harmonic mean, given by the formula $2PR/(P+R)$ 
			%\[\frac{2PR}{P+R}\]
			}
			\end{itemize}
	...
	\label{sec:classification:features}
		...
		To evaluate feature performance, classifiers were trained using various subsets of the complete feature set;  the various feature sets used in the experiments are listed in \cref{tab:features:sets}. For each of these feature combinations, classifiers were trained on each of the 10 training sets created as just described, and tested on the corresponding test set. 
		
		\TODO{Things to mention:
		\begin{itemize}
			\item Random FG data splits
			\item Compare kappas to inter-annotator kappas
		\end{itemize}
		}
		...
		\begin{table}
			\centering
			\caption{Results of experiments with prosodic features \TODO{explain stats} \TODO{remove [incorrect] stats?}}
...
PROSODY 	&	67.52	&	0.25	&	0.71	&	0.89	&	0.79	&	0.58	&	0.37	&	0.43	\\
		...
		\begin{table}
			\centering
			\caption{Results of experiments with speaker and word features \TODO{remove [incorrect] stats?} \TODO{Redo with DUR-F0 instead of PROSODY}}
			\begin{tabularx}{\textwidth}{lXXXXXXXX}			
