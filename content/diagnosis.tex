...
%, and/or other features informed by research on speaker identification (e.g. \cite{Shriberg2005}). \TODO{examples of speaker ID features}
	...
	%
	 ...
	 Given the relative novelty of this type of diagnosis for prosodic errors, diagnosis by classification was of particular interest in this thesis project. The classification experiments conducted in this work are described in the sections that follow. \TODO{\textit{rephrase this paragraph}}. \TODO{outline what experiments were done}
	 
	\subsection{Data and method}
		
		In addition to the motivation of analyzing the frequency and distribution of lexical stress errors in L2 German speech by L1 French speakers, another motivation behind the annotation of these errors in a subset of the IFCASL corpus (described in \cref{chap:lexstress}) was the creation of labeled data for a supervised machine learning approach to diagnosing learner errors. Using the WEKA machine learning toolkit \citep{Hall2009}, classifiers were trained to predict a label (e.g. [correct] or [incorrect]) for a given learner utterance using the gold-standard labels from the annotated sub-corpus (see \cref{sec:agreement:gold}) as training data. The accuracy of these classifiers was assessed by using held-out portions of the annotated data as test sets, and performing cross-evaluation on multiple train/test splits of the data. Details of the features, classifiers, and data splits used in each experiment are given in the relevant section below. \TODO{reword now that using only 1 classifier}
