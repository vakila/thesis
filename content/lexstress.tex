...
%			\end{figure}
		...
		However, choosing gold-standard labels for the remaining 
		%116 
		utterances was a less straightforward task. 
		
				\TODO{Explain how a final label was decided for each token}
		
		
		At each step $i$, any tokens whose set of assigned labels fits condition $C_i$ are assigned the gold-standard label described in column $s_{\text{gold}}(t)$. Column $N(C_i)$ gives the number of tokens labeled at this step, i.e. the number matching $C_i$. The rightmost column $N(C_{1 \dots i})$ gives the total number of tokens which have been assigned a gold-standard label at the end of step $i$, i.e. the number of tokens matching $C_i$ or any previous condition.
		
		\begin{table}[htb]
		\caption[Rules for choosing a gold-standard label for a given token]{Rules for choosing a gold-standard label $s_{\text{gold}}(t)$ for a given token $t$. }
			\begin{tabularx}{\textwidth}{lXlrr}
			\toprule
			%Step (
			$i$
			& Condition ($C_i$) & $s_{\text{gold}}(t)$ & $N(C_i)$ & $N(C_{1 \dots i})$ \\
			\midrule
			...
			4.& [bad\_nsylls] is one of the competing labels, 
			%ignore it; \newline if 
			and there is only one other competing label $s_{\text{only}}$
			%remains 
			& 
			%$s_{\text{gold}}(t) \gets 
			$s_{\text{only}}$ & 17 & 601 \\
			...
			7.& There are two competing labels: [correct] and \mbox{[incorrect]} &
			[correct] & 40 & 668\\
